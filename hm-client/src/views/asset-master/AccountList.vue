<template>
  <el-card class="box-card" shadow="always" body-style="padding:10px 20px">
    ...
    <el-table :data="options.accounts" border style="width: 100%">
      ...
      <el-table-column prop="name" label="口座名" width="180"></el-table-column>
    ...
    <el-dialog title="新規登録" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        <el-form ref="form" :model="form" label-width="200px" size="medium">
          ...
          <el-form-item label="口座カテゴリ">{{
            form.accountCategoryId
          }}</el-form-item>
          <el-form-item label="口座名">
  ...
  methods: {
    ...
    onClickEdit: function(selectedId) {
      ...
      if (selectedId === 0) {
        ...
        this.form = {
          id: getData.id,
          accountCategoryId: getData.accountCategoryId,
          name: getData.name
        }
      }
    },
    onClickRegistAPI: function() {
      ...
      var request = {
        id: this.form.id,
        accountCategoryId: this.form.accountCategoryId,
        name: this.form.name
      }
